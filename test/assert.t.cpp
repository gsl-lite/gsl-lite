...
bool assertAtAudit( bool x, std::source_location const & loc = std::source_location::current() ) { gsl_AssertAtAudit( loc, x ); return x; }
#endif
...
std::string colorToStringAt( Color color, std::source_location const & loc = std::source_location::current() )
{
    ...
    gsl_FailFastAt( loc );  // this should keep the compiler from issuing a warning about not returning a value
}
#endif
