...
#define gsl_DETAIL_CFG_3STATE_VALUE_2  1
#define gsl_DETAIL_CFG_3STATE_VALUE_1  1
#define gsl_DETAIL_CFG_3STATE_VALUE_0  1
...
#define gsl_CHECK_CFG_3STATE_VALUE_( x )  gsl_CONCAT_( gsl_DETAIL_CFG_3STATE_VALUE_, x )
...
# if ! gsl_CHECK_CFG_3STATE_VALUE_( gsl_CONFIG_USE_CRT_ASSERT_FUNCTION )
#  pragma message (__FILE__ "(" gsl_STRINGIFY( __LINE__ ) "): warning: invalid configuration value gsl_CONFIG_USE_CRT_ASSERT_FUNCTION=" gsl_STRINGIFY(gsl_CONFIG_USE_CRT_ASSERT_FUNCTION) ", must be 0, 1, or 2")
# endif
#else
# if defined( _MSC_VER ) || defined( __GNUC__ ) || defined( __clang__ ) || defined( __NVCOMPILER )
#  define gsl_CONFIG_USE_CRT_ASSERT_FUNCTION  2  // default
# else
...
#  elif gsl_CONFIG( USE_CRT_ASSERT_FUNCTION )
...
#  define    gsl_FAILFAST_()                    ( ::gsl_lite::detail::fail_fast_trace( "unreachable", gsl_nullptr, 0 ) )
...
public:
    ...
    : data_( T( std::move( other ) ) )
    {
        ...
        {
            gsl_AssertAt( loc, data_.ptr_ != nullptr );
        }
    }
#endif // gsl_BASELINE_CPP20_
...
# endif // gsl_HAVE( MOVE_FORWARD )
    ...
    gsl_api constexpr /*implicit*/ not_null( C const ( & literal )[N], std::source_location const & = std::source_location::current() ) gsl_noexcept
        : data_( literal )
    {
    }
#else // ! gsl_BASELINE_CPP20_
...
# endif
...
#if gsl_BETWEEN( gsl_COMPILER_MSVC_VERSION, 1, 140 ) && gsl_HAVE( MOVE_FORWARD )
...
#endif // gsl_BETWEEN( gsl_COMPILER_MSVC_VERSION, 1, 140 ) && gsl_HAVE( MOVE_FORWARD )
    ...
    gsl_noexcept_not_testing // we want to be nothrow-swappable despite the precondition check
    {
        ...
        if constexpr ( ! detail::has_trivial_move_v<T> )
        {
            gsl_Assert( lhs.data_.ptr_ != nullptr );
            gsl_Assert( rhs.data_.ptr_ != nullptr );
        }
#else // ! gsl_BASELINE_CPP20_
        accessor::check( lhs );
        accessor::check( rhs );
...
# if gsl_BASELINE_CPP20_
        ...
        return data_.ptr_( static_cast<Ts&&>( args )... );
# else // ! gsl_BASELINE_CPP20_
