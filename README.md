...
#### `gsl_CONFIG_NOT_NULL_DIRECT_CTOR=1`
Define this macro to 0 to forbid direct construction of `not_null<>` from the underlying type. **Default is 1.** If direct construction is suppressed, the *gsl-lite*-specific function `require_not_null()` must be used to coerce a nullable pointer into a non-nullable one.
